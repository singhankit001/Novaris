...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
    ...
    return (
        <div className="flex h-screen overflow-hidden bg-transparent text-white relative z-10">
            {/* Desktop Sidebar */}
            <aside
                className={`hidden md:flex flex-col border-r border-white/10 bg-[#09090b]/55 backdrop-blur-xl transition-all duration-300 z-50 ${isCollapsed ? 'w-20' : 'w-64'}`}
            >
                <div className="p-6 flex items-center justify-between">
                    {!isCollapsed && (
                        <Link href="/" className="flex items-center gap-2 group">
                            <div className="w-8 h-8 flex items-center justify-center group-hover:scale-110 transition-transform">
                                <Image src="/no-bg-novaris.png" alt="Logo" width={32} height={32} className="object-contain" />
                            </div>
                            <span className="font-bold text-xl tracking-tight">Novaris</span>
                        </Link>
                    )}
                    <button
            ...
            <header className="md:hidden fixed top-0 left-0 right-0 h-16 bg-zinc-950/80 backdrop-blur-xl border-b border-white/5 px-4 flex items-center justify-between z-40">
                <Link href="/" className="flex items-center gap-2">
                    ...
                    <span className="font-bold text-lg">Novaris</span>
                </Link>
                <button
            ...
            <main className="flex-1 pt-16 md:pt-0 overflow-auto relative bg-transparent">
