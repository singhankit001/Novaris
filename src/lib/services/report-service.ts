...
export function buildOutreachPack(scan: StoredScan, shareUrl: string): OutreachPackData {
    ...
    if (isClean) {
        outreachMessage = [
            ...
            "",
            `I ran a security health check through Novaris and it returned zero vulnerabilities. It's rare to see a repository this well-hardened, and I wanted to share the clean report with you.`,
            "",
            "RESOURCES",
            "",
            ...
            "",
            `You might want to add a Novaris verified badge to your README to show off the codebase quality!`,
            "",
            `Best regards,`,
        ].join("\n");
    } else if (!isHighRisk) {
        outreachMessage = [
            ...
            "",
            `I ran a quick hygiene scan with Novaris and noticed ${total} minor areas where the security posture could be polished (${medium} medium / ${low} low). Nothing critical, just a few maintenance improvements to keep things airtight.`,
            "",
            "RESOURCES",
            "",
            `Repo Profile: ${repoProfileUrl}`,
        ...
        outreachMessage = [
            ...
            "",
            `While doing a deep dive, I ran a Novaris security scan and identified ${total} findings that require your attention (${high} high / ${critical} critical). Sharing this privately first so you can triage safely before any public disclosure.`,
            "",
            strongestFinding
