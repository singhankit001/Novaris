...
export default function InteractiveDemo() {
    ...
    useEffect(() => {
        ...
        const sequence = [
            { step: 1, delay: 500 }, // Wait, start typing
            { step: 2, delay: 1000 }, // Finished typing, hit enter, show loading
            { step: 3, delay: 600 }, // AI starts analyzing context
            { step: 4, delay: 800 }, // AI starts streaming answer
            { step: 5, delay: 4000 }, // Answer complete, wait before restart
            { step: 0, delay: 500 }, // Reset and next scenario
        ];
    ...
    return (
        <section ref={containerRef} className="py-20 px-4 relative z-10 w-full max-w-5xl mx-auto flex flex-col items-center">
            ...
            <div className="w-full relative rounded-2xl overflow-hidden bg-zinc-950/60 backdrop-blur-2xl border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.5)] shadow-blue-900/30">
                {/* macOS window top bar */}
                <div className="flex items-center px-4 py-3 bg-white/5 border-b border-white/10 transition-colors duration-500">
                ...
                <div className="p-6 md:p-8 space-y-6 min-h-[450px] overflow-hidden flex flex-col">
                    ...
                    <div className="flex gap-4 items-start w-full relative z-10">
                        <div className="p-2 bg-blue-500/20 backdrop-blur-md rounded-lg shrink-0 border border-blue-400/30 overflow-hidden shadow-[0_0_15px_rgba(59,130,246,0.3)]">
                            <Image src="/user-avatar.png" alt="User" width={24} height={24} className="w-6 h-6 rounded-sm object-cover" />
                        </div>
                        <div className="bg-white/5 backdrop-blur-md border border-white/10 p-4 rounded-xl rounded-tl-none w-full text-zinc-200 font-mono text-sm shadow-xl relative">
                    ...
                    <AnimatePresence mode="wait">
                        ...
                        {visibleStep >= 4 && (
                            ...
                            >
                                <div className="p-1 bg-white/5 backdrop-blur-md rounded-lg shrink-0 border border-white/10 shadow-[0_0_20px_rgba(255,255,255,0.1)] overflow-hidden flex items-center justify-center w-10 h-10">
                                    <Image src="/1080x1080.png" alt="Novaris" width={32} height={32} className="w-8 h-8 rounded-md object-cover" />
                                </div>
                                ...
                                <div className="flex-1 space-y-4">
                                    ...
                                    <div className="bg-gradient-to-br from-white/[0.03] to-white/[0.01] backdrop-blur-xl border border-white/10 p-5 rounded-2xl rounded-tl-none w-full text-zinc-200 shadow-xl relative overflow-hidden">
                                        {/* Dynamic accent line */}
                ...
                {/* Glow Effects */}
                <div className={`absolute top-1/2 left-1/4 w-48 h-48 blur-[80px] rounded-full pointer-events-none transition-colors duration-1000 ${currentScenario.type === "chat" ? "bg-blue-500/20" :
                    currentScenario.type === "architecture" ? "bg-purple-500/20" :
                        "bg-red-500/20"
                    }`} />
                <div className={`absolute bottom-1/4 right-1/4 w-48 h-48 blur-[80px] rounded-full pointer-events-none transition-colors duration-1000 ${currentScenario.type === "chat" ? "bg-purple-500/20" :
                    currentScenario.type === "architecture" ? "bg-blue-500/20" :
                        "bg-orange-500/20"
                    }`} />
            </div>
